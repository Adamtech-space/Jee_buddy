...
class StorageManager:
    """Manages video storage and cleanup in Supabase using database configuration"""

    def __init__(self):
        # Use the new database configuration environment variables
        ...
        # Construct the connection URL using the database credentials
        connection_url = f"postgresql://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"
        self.connection_url = connection_url
        
        self.bucket_name = 'manim-animations'
        # Initialize the Supabase client with the constructed connection URL.
        # Here we pass None for the key. If your workflow requires a key, adjust accordingly.
        self.client = create_client(connection_url, None)

    async def upload_video(self, video_path: str) -> str:
        """Upload video to Supabase storage and return public URL"""
        try:
            file_name = os.path.basename(video_path)
            # Upload to Supabase storage using the constructed client
...
class ManimRenderer:
        ...
        try:
            ...
            with open(abs_script_path, 'r', encoding='utf-8') as f:
                ...
                if 'class ' not in content:
                    logger.error("No class found in the script.")
                    return None
    ...
    def _setup_ffmpeg(self):
        """Setup ffmpeg path"""
        ffmpeg_path = Path("C:\\ffmpeg\\bin")  # Set the correct path to your ffmpeg installation
        if ffmpeg_path.exists():
            os.environ["PATH"] = f"{ffmpeg_path};{os.environ['PATH']}"
            logger.info("FFmpeg path set successfully.")
        else:
            logger.error("FFmpeg path does not exist. Please check the path.")
